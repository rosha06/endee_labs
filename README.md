...
**Symptoms**: API returns 500 error, logs show connection refused  
**Solution**:
```bash
# Check if Endee is running
docker ps | findstr endee
...
**Symptoms**: All tickets classified as "Unclassified" with 0% confidence  
**Solution**:
```bash
# Verify index exists
...
# Check vector count
curl http://localhost:8080/api/v1/index/support_tickets/stats
```
...
# Or change Endee port in docker-compose-endee.yml
ports:
